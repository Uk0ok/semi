...
<%
	...
	List<recipeVo> rdatalist = (List<recipeVo>)request.getAttribute("data"); // attribute는 object라서 ArrayList로 형변환
	for(recipeVo rrv : rdatalist) { 
...
<title>레시피 게시판 목록</title>
		...
		<body>
							...
							<div class="item">
								...
								<div class="tit">
									<a href="${pageContext.request.contextPath}/recipe/viewRecipe?no=${rrv.rpostNo}&pageNo=${currentPage}">
										<img src="${pageContext.request.contextPath}/img/recipeBoard/${rrv.rthumbnail}"	width="200" height="150"> 
										${rrv.rpostName}
									</a>
								</div>
								<div class="writer">${loginUser.userId}</div>
			...
			<c:if test="${!empty loginUser}">
			<a type="button" href="registerRecipe"
				class="btn-primary btn writing"
				style="background-color: darkolivegreen; border-color: darkolivegreen;">글쓰기</a>
			</c:if>
		</body>
	</div>
</body>
