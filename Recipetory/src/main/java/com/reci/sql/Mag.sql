...
CREATE TABLE TB_BOARD_RECEIPT (
	...
	DEL_YN	CHAR(1)	DEFAULT 'N' CHECK(DEL_YN IN('Y', 'N')),
	FMOD_DATE	TIMESTAMP,
    CONSTRAINT PK_TB_BOARD_RECEIPT PRIMARY KEY(POST_NO),
    CONSTRAINT FK_TB_BOARD_RECEIPT FOREIGN KEY(USER_NO)
    REFERENCES TB_USER(USER_NO)
);
...
CREATE TABLE TB_COMMNET_R (
	...
	MOD_YN	        CHAR(1)	DEFAULT 'N' CHECK(MOD_YN IN('Y', 'N')),
	DEL_YN	        CHAR(1)	DEFAULT 'N' CHECK(DEL_YN IN('Y', 'N')),
    CONSTRAINT PK_TB_COMMENT_R PRIMARY KEY(COMM_NO),
    CONSTRAINT FK_TB_COMMENT_R FOREIGN KEY(POST_NO)
    REFERENCES TB_BOARD_RECEIPT(POST_NO),
    FOREIGN KEY (USER_NO) REFERENCES TB_BOARD_RECEIPT
);
...
CREATE TABLE TB_COMMENT_R2 (
	...
	MOD_YN	    CHAR(1)	    DEFAULT 'N' CHECK(MOD_YN IN('Y', 'N')),
	DEL_YN	    CHAR(1)	    DEFAULT 'N' CHECK(DEL_YN IN('Y', 'N')),
    --CONSTRAINT FK_TB_COMMENT_R2 FOREIGN KEY(COMM_NO)
    --REFERENCES TB_COMMENT_R(COMM_NO),
    --FOREIGN KEY (POST_NO) REFERENCES TB_COMMENT_R,
    --FOREIGN KEY (USER_NO) REFERENCES TB_COMMENT_R --제약 조건 못넣음. 알터로 해야할듯.
);
